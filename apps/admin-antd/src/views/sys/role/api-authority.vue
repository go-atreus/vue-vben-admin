...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    ...
    const result = await defRoleService.UpdateApiAuthority({
...
async function getApiData() {
  try {
    treeApiData.value = [];
    const apiData = await defApiService.List({
      page: 1,
      pageSize: 10_000,
    });
    tempApiList = apiData;
    ...
    const checkedData = await defRoleService.GetApiAuthority({ id: roleId.value });
    checkedKeys.value =
      checkedData.data === null
...
function convertApiTreeData(params: ApiInfo[]): DataNode[] {
  ...
  for (const param of params) {
    apiMap.set(param.apiGroup, param.serviceName);
