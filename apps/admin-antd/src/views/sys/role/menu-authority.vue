...
const [Modal, modalApi] = useVbenModal({
  ...
  onConfirm: async () => {
    if (checkedKeys.value.checked !== undefined) {
      const result = await defRoleService.UpdateMenuAuthority({
...
async function getMenuData(roleId: number) {
  try {
    ...
    const checkedData = await defRoleService.GetMenuAuthority({ id: roleId });
    checkedKeys.value = checkedData.menuIds;
    expandedKeys.value = data.data.map((val, _idx, _arr) => val.id);
  } catch {
    // console.log(error);
  }
}
